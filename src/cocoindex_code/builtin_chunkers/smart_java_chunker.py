"""Java chunker — splits by top-level class/interface/enum/method boundaries via tree-sitter.

Activates for all .java files. Uses tree-sitter-java to identify compilation-unit-level
declarations (classes, interfaces, enums, records). Falls back to RecursiveSplitter
for files that fail parsing.
"""
...
def _ast_chunks(
    ...
    # Header: text before the first top-level declaration (package/import declarations).
...
def java_chunker(path: Path, content: str) -> tuple[str | None, list[Chunk]]:
    """Java chunker using tree-sitter; falls back to generic splitter on failure."""
