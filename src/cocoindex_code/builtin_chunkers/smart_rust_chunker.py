"""Rust chunker — splits by top-level function/struct/trait/impl boundaries via tree-sitter.

Activates for all .rs files. Uses tree-sitter-rust to identify crate-level
declarations (functions, structs, traits, impls, enums, modules). Falls back
to RecursiveSplitter for files that fail parsing.
"""
...
def _ast_chunks(
    ...
    # Header: text before the first top-level declaration (use statements, mod declarations).
...
def rust_chunker(path: Path, content: str) -> tuple[str | None, list[Chunk]]:
    """Rust chunker using tree-sitter; falls back to generic splitter on failure."""
    if not tree_sitter_available():
        ...
        content_bytes = content.encode("utf-8")
        return "rust", fallback or [
            Chunk(text=content, start=_pos(0, content_bytes), 
                  end=_pos(len(content_bytes), content_bytes))
        ]

    try:
        chunks = _ast_chunks(content, "rust", _RUST_SPLIT_NODES)
        if chunks:
            return "rust", chunks
