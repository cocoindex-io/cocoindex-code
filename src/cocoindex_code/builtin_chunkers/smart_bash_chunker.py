"""Bash chunker — splits by top-level function boundaries via tree-sitter.

Activates for all .sh, .bash, .zsh files. Uses tree-sitter-bash to identify
top-level functions and commands. Falls back to RecursiveSplitter for files
that fail parsing.
"""
...
def _ast_chunks(
    ...
    # Header: text before the first top-level declaration (shebang, initial comments/setup).
...
def bash_chunker(path: Path, content: str) -> tuple[str | None, list[Chunk]]:
    """Bash chunker using tree-sitter; falls back to generic splitter on failure."""
